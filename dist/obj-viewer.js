...
export class ObjViewer extends HTMLElement {
    ...
    setupEventListeners() {
        const objFileInput = this.shadowRoot?.getElementById('objFileInput');
        const mtlFileInput = this.shadowRoot?.getElementById('mtlFileInput');
        if (objFileInput) {
    ...
    showMeshModal(meshInfo) {
        const meshName = this.shadowRoot?.getElementById('meshName');
        const visibilityToggle = this.shadowRoot?.getElementById('visibilityToggle');
        const colorPicker = this.shadowRoot?.getElementById('colorPicker');
        if (meshName)
            meshName.textContent = meshInfo.name;
        if (visibilityToggle)
            visibilityToggle.checked = meshInfo.visible;
        if (colorPicker) {
    ...
    loadObjFile(objFile, mtlFile = null) {
    ...
    loadObjWithoutMaterials(objFile) {
        const objUrl = URL.createObjectURL(objFile);
        this.objLoader.load(objUrl, (object) => {
            this.onObjLoaded(object);
            this.showLoading(false);
    ...
    showLoading(show) {
        let loadingElement = this.shadowRoot?.querySelector('.loading');
        if (show) {
            if (!loadingElement) {
    ...
    loadModelFromUrl(objUrl, mtlUrl) {
        this.showLoading(true);
        if (mtlUrl) {
            this.mtlLoader.load(mtlUrl, (materials) => {
                materials.preload();
    ...
    captureViewport(format = 'png', quality = 0.9) {
        this.renderer.render(this.scene, this.camera);
        const canvas = this.renderer.domElement;
        return canvas.toDataURL(`image/${format}`, quality);
    }
    downloadViewport(filename = 'screenshot.png') {
